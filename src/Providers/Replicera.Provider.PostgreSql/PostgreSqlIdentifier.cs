...
{
    public const int MaximumByteLength = 63;

    public static string Normalize(string sourceName)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(sourceName);
        var normalized = new string(sourceName
            .Select(character => char.IsLetterOrDigit(character) || character == '_' ? char.ToLowerInvariant(character) : '_')
            .ToArray());
    ...
    public static string Qualified(string schema, string table) =>
        $"{Quote(Normalize(schema))}.{Quote(Normalize(table))}";
}
